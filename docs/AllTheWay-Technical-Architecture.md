...
- **Model**: Gemini 3.1 Flash Live via the Live API — audio-to-audio, not a speech-to-text-to-LLM-to-text-to-speech chain, which is what preserves conversational latency and vocal nuance (tone, pacing, interruption handling).
...
- **Transport**: for production-grade session orchestration and routing, a bidirectional WebRTC integration (e.g., via LiveKit Agents, a documented Live API partner integration) sits between the client and the Live API session, rather than a raw client-to-Gemini connection — this is what supports proper session security (ephemeral tokens) and multi-region routing at scale.
