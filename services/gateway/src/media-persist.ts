...
export function mediaFromConnectorTask(task: unknown): MediaPayload {
  ...
  const walk = (node: unknown, depth: number): void => {
    ...
    if (typeof rec.error === "string" && rec.error.trim() && !error) {
      error = rec.error.trim();
    }
    if (typeof rec.content === "string" && rec.content.length > 80) {
      const mime = typeof rec.mimeType === "string" ? rec.mimeType : "";
      if (mime.startsWith(IMAGE_PREFIX) || mime.startsWith(VIDEO_PREFIX)) {
        content = rec.content;
        mimeType = mime;
      }
    }
    if (typeof rec.model === "string" && rec.model.trim() && !model) {
      model = rec.model.trim();
    }
    if (typeof rec.data === "string" && rec.data.startsWith("{")) {
      try {
        walk(JSON.parse(rec.data), depth + 1);
      } catch {
        /* not JSON */
      }
    }
