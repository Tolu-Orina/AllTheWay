    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-2 p-6 text-center">
        ...
        <p className="max-w-[22rem] text-[12.5px] leading-relaxed text-muted-foreground">
          Ask for a draft in the conversation and it will appear here, where you
          can correct it.
        </p>
      </div>
    );
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {imageUrl ? (
          <figure className="flex flex-col gap-2">
            <img src={imageUrl} alt={`Version ${version.n}`} className="w-full rounded-brand border" />
            {version.producedBy === "agent" ? (
              // The label belongs to whatever surface shows generated media,
              // so the hook exists before Phase C fills it in.
              <figcaption className="text-[12px] text-muted-foreground">
                Generated by AllTheWay
              </figcaption>
            ) : null}
          </figure>
        ) : (
